...
void fn_list_employee (GtkWidget *liste){
	...
	if (store == NULL){
		...
		renderer = gtk_cell_renderer_text_new ();
		column = gtk_tree_view_column_new_with_attributes (" Mot de passe",renderer,"text",SECTEUR,NULL);
		gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);
...
*/
			...
			while (fread (&aux,sizeof (employee),1,f) != 0 && test == 0){
				if (strstr (aux.cin,cin_rech) != NULL || strstr (aux.nom,cin_rech) != NULL || strstr (aux.pren,cin_rech) != NULL || strstr (aux.genre,cin_rech) != NULL){
					gtk_list_store_append (store, &iter);
...
void fn_list_conge_em (GtkWidget *liste){
